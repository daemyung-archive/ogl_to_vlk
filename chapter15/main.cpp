...
private:
    void init_signals_()
    {
        window_->startup_signal.connect(this, &Chapter15::on_startup);
        window_->shutdown_signal.connect(this, &Chapter15::on_shutdown);
        window_->render_signal.connect(this, &Chapter15::on_render);
    }
    ...
    void init_fence_()
    {
        ...
        // 커맨드 버퍼가 모두 처리 됬었는지를 알기 위해 사용된 펜스는 처음에 시그널 상태면 어플리케이션의 로직이 단순해집니다.
    ...
    {
        // 데이터가 변경되지 않는 버텍스 버퍼, 인덱스 버퍼와는 달리 유니폼 버퍼의 내용은 매 프레임마다 변경됩니다.
        // 각 프레임마다 유니폼 버퍼를 생성하지 않으면 렌더링 문제가 발생할 수 있습니다.
        // 왜냐하면 커맨드 버퍼를 큐에 제출했을 뿐 GPU가 처리하지는 않았기 때문입니다.
        // 그러므로 한 개의 유니폼 버퍼를 사용한다면 제출한 커맨드 버퍼가 처리될 때 다음 프레임을 위한 데이터가 사용될 수 있습니다.
    ...
    void init_swapchain_images_()
    {
        ...
        // 이미지 레이아웃을 변경하기 위한 커맨드 버퍼를 정의합니다.
        VkCommandBufferAllocateInfo allocate_info {};
    ...
    void on_render()
    {
        ...
        // 현재 프레임에 해당하는 유니폼 버퍼와 메터리얼 디스크립터 셋을 사용합니다.
        ...
        // 현재 프레임에 해당하는 텍스처 디스크립터 셋을 사용합니다.
        ...
        vkCmdBindDescriptorSets(command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
                                pipeline_layout_, 0, 1, &material_descriptor_set,
                                0, nullptr);
        vkCmdBindDescriptorSets(command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
                                pipeline_layout_, 1, 1, &texture_descriptor_set,
                                0, nullptr);
        ...
        // 다음 프레임에 해당하는 리소스를 참조하기 위해 프레임 인덱스를 1만큼 증가시킵니다.
